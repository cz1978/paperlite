...
只给 agent 用 MCP 时，不需要 Docker，也不需要 `docker compose up --build`。安装 `python -m pip install -e ".[mcp]"`，再让 agent 启动 `python -m paperlite.mcp_server` 就够了。
...
- PaperLite 自己的 LLM、AI 筛选或 brief 翻译未配置，不代表 QClaw、Hermes、OpenClaw 这类宿主 agent 不能继续。不要说缓存论文丢失、数据库重建、重装/reset，除非 PaperLite 工具明确返回了这个事实。
