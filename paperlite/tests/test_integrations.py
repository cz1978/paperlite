...
def test_agent_manifest_declares_reserved_interfaces():
    ...
    assert "display_title" in result_policy["paper_fields"]
    assert "title_zh" in result_policy["paper_fields"]
    assert "title_en" in result_policy["paper_fields"]
    assert "title_original" in result_policy["paper_fields"]
    assert "title" in result_policy["paper_fields"]
    assert "identifier" in result_policy["paper_fields"]
    assert "identifier_label" in result_policy["paper_fields"]
    ...
    assert "include both titles" in result_policy["visible_title_rule"]
    assert "DOI/arXiv-style identifier" in result_policy["visible_title_rule"]
    assert "Do not display raw English" in result_policy["visible_title_rule"]
