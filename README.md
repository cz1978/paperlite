...
Agent result rule: the user's current prompt wins. If no different format is requested, after crawling, organizing, filtering, or ranking papers, the agent should state the scope first: discipline, source key/name, date range, query, run status, warnings, and total count. Then it should list the actual papers. If there are 15 or fewer papers, list all of them with title, source/venue, date, URL/DOI, reason, and a brief abstract/summary. If more than 15 papers match, list at most 15, state the remaining count, and ask whether to AI-rank/optimize or add search keywords to narrow the set. In Chinese brief answers, include both titles and the identifier: use `paper.display_title`, `paper.title_zh`, or `paper.brief_translation.title_zh` as the Chinese title first, then include `paper.title_original` or `paper.title_en` as the English original, and include `paper.identifier_label` + `paper.identifier` for DOI/arXiv-style IDs. If the Chinese title is missing, translate `paper.title_original` with the host model before displaying it. Do not show raw English `paper.title` as the only item heading. Use `paper.brief_translation.cn_flash_180` first for the one-sentence summary; if missing, include a host-model Chinese abstract/summary for every listed paper unless the user asks otherwise. Highlights come after the list, never instead of it.
